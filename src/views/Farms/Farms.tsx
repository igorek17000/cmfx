      ...
      case 'multiplier':
        return orderBy(
          ...
          (farm: FarmWithStakedValue) =>
            Number(farm.multiplier.slice(0, -1)),
          'desc'
        );
      case 'earned':
        return orderBy(
  ...
  const farmsList = useCallback(
    (farmsToDisplay): FarmWithStakedValue[] => {
      const cakePriceVsBNB = new BigNumber(
        farmsLP.find((farm) => farm.pid === CAKE_POOL_PID)
          ?.tokenPriceVsQuote || 0
      );
      let farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.map(
        (farm) => {
          ...
          const cakeRewardPerBlock = CAKE_PER_BLOCK.times(
            farm.poolWeight
          );
          const cakeRewardPerYear = cakeRewardPerBlock.times(
            BLOCKS_PER_YEAR
          );
          ...
          switch (column.name) {
            ...
            case 'apr':
              if (a.original.apr.value && b.original.apr.value) {
                return (
                  Number(a.original.apr.value) -
                  Number(b.original.apr.value)
                );
              }
            ...
            case 'earned':
              return (
                a.original.earned.earnings -
                b.original.earned.earnings
              );
            default:
              return 1;
          }
